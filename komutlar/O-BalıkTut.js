...
exports.run = async (client, message, args) => {
  ...
  if (crime !== null && timeout - (Date.now() - crime) > 0) {
    ...
    if (para < gerekenpara) return message.channel.send(
      new Discord.MessageEmbed()
        .setColor("RED")
        .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
        .setDescription(`Balık tutmak için balıkçı dayıdan olta kiralamalısın! Gereken Para: ${gerekenpara} 💸`)
    );
    ...
    const cümleler = [
      "Balık oltadan kaçtı",
      "Hırsızın biri balığını çaldı",
      "Martının biri gelip balığını kaptı"
    ];
